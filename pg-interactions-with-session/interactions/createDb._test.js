...
export async function rebuildDbSchema() {
  ...
  try {
    ...
    const sql1 = await promisify(fs.readFile)(path.join(evolutionsDir, '001.sql'));
    await client.query(sql1.toString());
    const sql2 = await promisify(fs.readFile)(path.join(evolutionsDir, '002.sql'));
    await client.query(sql2.toString());
  } finally {
    await client.end();
  }
}
