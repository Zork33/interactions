...
export default function (services) {
  ...
  function process(args) {
    ...
    function _buildNext() {
      const statement =
        `with ia as (update interaction set lock = now() + (${lockPeriod} * interval '1 ms') ` +
        ...
        `returning *) select ia.*, s.bot, s.channel, s.user_id, now()::timestamp as _time from ia join session s on s.id = ia.session_id`;
