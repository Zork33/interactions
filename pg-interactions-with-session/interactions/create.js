...
export default function (services) {
  ...
  return async function create(args) {
    ...
    let {context, sessionId, bot, channel, userId, name, parentId, fromService, toService, action, innerAction, messageId, completed, error, cancelled, processAt, processIn, wait, ...options} = args;
    ...
    try {
      ...
      const fields = ['session_id', 'from_service', 'to_service', 'action', 'options', 'completed', 'failed', 'created', 'modified'];
      const params = [sessionId, fromService, toService, action, options];
      let values = ['$1', '$2', '$3', '$4', '$5', `${!!completed}`, `${isError}`, 'now()', 'now()'];
      ...
      const r = await connection.exec({
        statement:
          `with ia as (insert into interaction (${fields.join()}) values (${values.join()}) returning *) ` +
          `select ia.*, s.bot, s.channel, s.user_id from ia join session s on s.id = ia.session_id;`,
        params
      });
