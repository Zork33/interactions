...
test.serial(
  ...
  `который ещё не completed или cancelled, и у которых next_processing не в будущем`, async t => {
    ...
    const activeInter1 = await dataInteractions.create({
    ...
    const activeInter2 = await dataInteractions.create({
      sessionId: t.context.session.id,
      fromService: 'test',
      toService: `testSvc`,
      action: `do2`
    ...
    const activeInter3 = await dataInteractions.create({
      sessionId: t.context.session.id,
      fromService: 'test',
      toService: `testSvc`,
      action: `do3`
    ...
    dataInteractions.process({
      ...
      processor: async (interaction, testProcessFinished) => {
        ++callCount;
        t.is(interaction.sessionId, t.context.session.id);
        t.is(interaction.bot, t.context.session.bot);
    ...
    t.deepEqual(
      await dataInteractions.get({id: activeInter1.id}),
      {
        ...activeInter1,
        state: COMPLETED,
    ...
    t.deepEqual(
      await dataInteractions.get({id: activeInter2.id}),
      {
        ...activeInter2,
        state: FAILED,
    ...
    t.deepEqual(
      await dataInteractions.get({id: activeInter3.id}),
      {
        ...activeInter3,
        state: CANCELLED,
...
test.serial(`2.3 Проверяем что работает ограничение на количество обработок в единицу времени`, async t => {
  ...
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // должен сработать первый nextProcess(), и поставить следующий nextProcess в тестовую очередь

  t.context.clock.tick(100); // 200ms
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // должен сработать nextProcess(), и поставить следующий nextProcess в тестовую очередь
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // плюс один вызов, который не должен добавить ещё один process, так как уже два в работе
  ...
  processFinished.pop()();
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // ожидаем, окончания завершения process
...
test.serial(`2.4 Проверяем что работает ограничение на количество одновременно обрабатываемых элементов`, async t => {
  ...
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // должен сработать nextProcess(), и поставить следующий nextProcess в тестовую очередь
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // должен сработать nextProcess(), и поставить следующий nextProcess в тестовую очередь
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // плюс один вызов, который не должен добавить ещё один process, так как уже три в работе
  ...
  processFinished.pop()();
  await t.context.awaitWithTimeout(dataInteractions._process._runAndAwaitAsyncs()); // ожидаем, окончания завершения process'ов
