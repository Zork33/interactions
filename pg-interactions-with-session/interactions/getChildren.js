...
export default function (services) {
  ...
  return async function getChildren(args) {
    ...
    const r = await postgres.exec({
      context,
      statement: `select ia.*, s.bot, s.channel, s.user_id from interaction ia join session s on s.id = ia.session_id where ia.parent_id = $1${extra} order by modified desc${limit ? ` limit ${limit}` : ''};`,
      params,
    });
